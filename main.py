...
class DokyOS:
    ...
    async def runtime_loop(self):
        ...
        while self.is_running:
            try:
                ...
                sleep_time = self.optimize_sleep_time(cycle_time)
    ...
    def optimize_sleep_time(self, cycle_time):
        """Optimize sleep time berdasarkan market volatility"""
        base_sleep = settings.BASE_CYCLE_TIME
        if cycle_time > base_sleep:
            return max(1, base_sleep - 0.5)  # Kurangi sleep jika cycle lambat
        return base_sleep
